...
if (
	...
	if (mix.inProduction()) {
		...
		fs.ensureDir(languages, function (err) {
			...
			wpPot({
				...
				bugReport: "https://github.com/smrafiz/my-plugin-boilerplate",
				src: "**/*.php",
				domain: "my-plugin-text-domain",
				destFile: "languages/my-plugin-boilerplate.pot",
			});
		});
	}
	...
	mix
		// Backend JS
		.js('src/js/backend.js', 'assets/js/backend.min.js').react()
	...
	// Backend CSS
	mix.postCss('assets/css/backend.min.css', 'assets/css/rtl/compiled-backend-rtl.css', [
		require('rtlcss'),
	]);
	mix.combine([
